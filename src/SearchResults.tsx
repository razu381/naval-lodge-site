      ...
      <div className="bg-white shadow-sm border-b border-stone-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4 text-sm">
            ...
            <span className="font-semibold text-amber-600">{searchLocation}</span>
            {checkIn && checkOut && (
              <>
            ...
            <span className="ml-auto text-slate-500">{sampleHotels.length} results found</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="space-y-6">
          {sampleHotels.map((hotel) => (
            <div
              key={hotel.id}
              className="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.06)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.1)] transition-all duration-300 overflow-hidden flex flex-col md:flex-row border border-stone-100/50"
            >
              {/* Image */}
              <div className="md:w-1/3 relative">
                ...
                />
                {hotel.discount && (
                  <div className="absolute top-4 left-4 bg-amber-500 text-slate-900 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                    {hotel.discount}
                  </div>
                )}
              </div>
              ...
              <div className="md:w-2/3 p-6 flex flex-col">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    <h3 className="font-serif text-2xl font-semibold text-slate-900 mb-2">{hotel.name}</h3>
                    <p className="text-slate-500 mb-3">{hotel.description}</p>
                ...
                {/* Amenity Icons */}
                <div className="flex flex-wrap gap-4 mb-4 text-slate-500">
                  ...
                  {hotel.amenities.length > 5 && (
                    <span className="text-xs text-slate-400">+{hotel.amenities.length - 5} more</span>
                  )}
                </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-4 h-4 ${i < Math.floor(hotel.rating) ? 'text-amber-500 fill-amber-500' : 'text-slate-300'}`}
                      />
                    ))}
                  </div>
                ...
                <div className="mt-auto flex items-center justify-between pt-4 border-t border-stone-100">
                  <div>
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl font-bold text-slate-900">{hotel.currency}{hotel.price}</span>
                      <span className="text-sm text-slate-500">/ night</span>
                    </div>
                    <p className="text-xs text-amber-600 mt-1">{hotel.discount}</p>
                  </div>
                  <button className="bg-amber-500 hover:bg-amber-400 text-slate-900 px-6 py-3 rounded-xl font-semibold transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/20 flex items-center gap-2 group">
                    View Details
